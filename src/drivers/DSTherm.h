...
public:
     ...
     *   time. If the resolution is not known or the chip is a fake one it's advisable to use max
     *   conversion time equal to 750 msecs (denoted by @ref MAX_CONV_TIME
     *   constant). @ref getConversionTime() may be used to retrieve
     *   conversion time for a given resolution for a original chips, for 
     *   ingenuine chips conversion times differ from specification and must be determined empirically.
     ...
     *   completed, otherwise waits @c MAX_CONV_TIME and returns. It may not
     *   work correctly for fake chips.
     * 
     ...
     *     - @c EC_NO_DEVS: No devices on the bus.
     *     - @c EC_CRC_ERROR: Scratchpad read with CRC error. It may be necessary
     *          to disable CRC checks for fake chips by defining CONFIG_CRC_DISABLE macro
     *          due to incorrect checksums generated by such chips.
     */
     ...
     * For ingenuine chips conversion time can be too short to obtain correct results.
     */
